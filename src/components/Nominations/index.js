import React from "react";
import {
  Typography,
  Card,
  CardActionArea,
  CardActions,
  CardContent,
  CardMedia,
  Button,
  makeStyles,
  Grid,
  TextField,
} from "@material-ui/core";
import Alert from "@material-ui/lab/Alert";
import { useSelector, useDispatch } from "react-redux";
import jwt from "jsonwebtoken";
import { MAX_NOMINATIONS, BASE_URL } from "../../utils/constants";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

const useStyles = makeStyles({
  root: {
    maxWidth: 345,
  },
  media: {
    height: 140,
  },
});

const Nominations = () => {
  const classes = useStyles();
  const state = useSelector((state) => state.movie);
  const dispatch = useDispatch();
  const removeNomination = (item) => {
    const newNominations = state.nominations.filter((i) => i !== item);

    localStorage.setItem(
      "aarsh-nominations",
      jwt.sign({ data: newNominations }, "aarsh-nominations")
    );
    toast(`Removed ${item.Title} (${item.Year}) from nominations`);
    dispatch({ type: "SET_NOMINATIONS", payload: newNominations });
  };

  return (
    <>
      <ToastContainer />
      <Typography variant="h5" component="h5">
        Nominations ({state.nominations.length})
      </Typography>
      <br />
      {state.nominations.length >= MAX_NOMINATIONS ? (
        <Alert severity="info">Max Nominations</Alert>
      ) : (
        <></>
      )}
      <br />
      <Grid container spacing={3}>
        {state.nominations.map((item, index) => {
          return (
            <Grid item xs={4} key={index}>
              <Card className={classes.root}>
                <CardActionArea>
                  <CardMedia
                    className={classes.media}
                    image={
                      item.Poster === "N/A"
                        ? "data:image/png;base64,"
                        : item.Poster
                    }
                    title={item.Title}
                  />
                  <CardContent style={{ height: "40px", overflow: "auto" }}>
                    <Typography variant="h6" component="h6">
                      {item.Title} ({item.Year})
                    </Typography>
                  </CardContent>
                </CardActionArea>
                <CardActions>
                  <Button size="small" color="primary">
                    <a
                      href={`https://www.imdb.com/find?q=${item.imdbID}`}
                      target="blank"
                    >
                      IMDb
                    </a>
                  </Button>
                  <Button
                    size="small"
                    color="primary"
                    onClick={() => removeNomination(item)}
                  >
                    Remove
                  </Button>
                </CardActions>
              </Card>
            </Grid>
          );
        })}
      </Grid>
      <br />
      <Typography>
        Share:{" "}
        <TextField
          style={{ minWidth: "100%" }}
          value={BASE_URL + localStorage.getItem("aarsh-nominations")}
        >
          {BASE_URL}/{localStorage.getItem("aarsh-nominations")}
        </TextField>
      </Typography>
    </>
  );
};

export default Nominations;
